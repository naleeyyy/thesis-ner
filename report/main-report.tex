...
%% to compile your LaTeX source file. If you want to use packages like minted that need
%% to execute shell commands, you may need to change the TeX program above to, e.g.
%% txs:///xelatex/[-shell-escape]
%% If your tool does not support these magic comments, you will need to make appropriate
%% manual choices.
%% 
...
%\usepackage{minted} %% alternative package for code listings (also see notice about TeX program above!)
\usepackage{csquotes}
...
\title{Building a Hand-Verified Named Entity Recognition Corpus for Albanian}

%% Command \titleshort{short title}: sets an abbreviated version of the report title for page heads
\titleshort{A Hand-Verified NER Corpus for Albanian}
...
\author{%
    ...
    \authornewline
    \authormail{k12337455@students.jku.at}
    \authormail{kahmeti123@gmail.com}
    \authornewline
}
...
\revisionblock{Supervisor: Timo Bertram\\
Institute of Machine Learning, Johannes Kepler University Linz}
...
%% DRAFT abstract. Two numbers are still missing because the annotation campaign is
%% running; they are marked \todonum below and must be filled before submission.
\newcommand{\todonum}[1]{\textbf{[#1]}}
\abstract{%
    Albanian's most widely used NER resource, WikiANN-sq, is built by automatic
    cross-lingual projection: its median sentence is 6 tokens against 20 in freshly
    sampled Wikipedia text. This report presents a hand-verified Albanian NER corpus of
    \todonum{N} sentences with inter-annotator agreement measured rather than assumed.
    Every entity carries both a full span and a proper-name head, so the boundary of
    Albanian's ambiguous \emph{X i Y} construction remains a reporting choice rather than a
    property of the data. The effect of large-language-model pre-annotation on annotators
    is measured in a between-subjects design (\todonum{result}). All model comparisons use
    bootstrap confidence intervals and paired significance tests.%
}

%% Command \keywords{text}: set the document keywords
\keywords{named entity recognition, Albanian, low-resource NLP, corpus annotation, active learning}
