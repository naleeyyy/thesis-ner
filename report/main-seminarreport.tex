...
%% 
\usepackage[seminarreport,fancyfonts,mathastext]{jkureport}
%% 
...
Note that you would often only use either this section or the short abstract on the title page.
...
This is an example for another section in your report.
...
Note that figures (just like tables and code listings, see \autoref{tab:nicetable} and \autoref{lst:examplecode}) are floating elements.
...
\caption{
    ...
    Captions may span multiple lines
...
\end{tabularx}
\end{table}
\begin{lstlisting}[language=c,caption={A code listing. Code listings usually have their caption below the code. Captions may span multiple lines},label=lst:examplecode,float]
for (int i = 0; i < 100; ++i) {
    printf("%3d\n", i);
}
\end{lstlisting}
...
%\end{minted}
%\caption{
%    A code listing. Code listings usually have their caption below the code.
%    Captions may span multiple lines
...
\end{quote}
\item You sometimes also paraphrase from multiple sources. For that purpose, the \string\cite\ command accepts a list of multiple comma-separated references. Do not add spaces in between them. Various analyses of the Tor network have been performed recently~\cite{bib:2021-hoeller-foci,bib:2021-hoeller-iiwas}. 
\end{itemize}
