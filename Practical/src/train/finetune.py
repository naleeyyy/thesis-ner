...
def predict(
    ...
    for start in range(0, len(examples), batch_size):
        ...
        for i, ex in enumerate(chunk):
            out.append(
                first_subword_tags(
                    enc.word_ids(batch_index=i), pred_ids[i], ID2LABEL, len(ex.tokens)
                )
            )
    return out
...
def train_one_seed(
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, int(total_steps * args.warmup_frac), total_steps
    )
...
def main() -> int:
    ...
    p.add_argument("--extra-wikiann-train", action="store_true",
    ...
    for seed in args.seeds:
        ...
        per_seed.append(
            {"seed": seed, "test_f1": test_f1, "best_dev_f1": hist["best_dev_f1"],
             "seconds": round(elapsed, 1), "epochs": hist["epochs"]}
        )
        print(f"  seed {seed}: test F1 {test_f1:.4f}  ({elapsed:.0f}s)\n")
    ...
    print(f"best seed {report['best_seed']}: {report['best_seed_f1']:.4f} "
